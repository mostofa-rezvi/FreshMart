  ...
  useEffect(() => {
    ...
    if (isCustomer && user.id) {
      ...
      socket.on("orderStatusUpdate", (data) => {
        ...
        alert(
          `Order ${data.orderId.substring(0, 8)} status updated to: ${
            data.newStatus
          }`
        );
    ...
    if (isVendor && user.id) {
      // Join room using vendor's User ID (which is available on user object)
      socket.emit("joinVendorRoom", user.id); // Backend expects vendor's userId
    ...
    return (
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-lg">
        <h1 className="text-3xl font-bold text-gray-800 mb-6">
          Order Details - #{singleOrder.id.substring(0, 8)}
        </h1>
        <p className="text-gray-600 mb-2">
          <strong>Status:</strong>{" "}
          <span
            className={`font-semibold ${getOrderStatusColor(
              singleOrder.status
            )}`}
          >
            {singleOrder.status}
        ...
        <p className="text-gray-600 mb-2">
          <strong>Total:</strong> {formatCurrency(singleOrder.totalAmount)}
        </p>
        <p className="text-gray-600 mb-2">
          <strong>Placed On:</strong> {formatDate(singleOrder.orderDate)}
        </p>
        <p className="text-gray-600 mb-2">
          <strong>Shipping To:</strong> {singleOrder.shippingAddress}
        </p>
        <p className="text-gray-600 mb-6">
          <strong>Contact:</strong> {singleOrder.contactPhone}
        </p>
